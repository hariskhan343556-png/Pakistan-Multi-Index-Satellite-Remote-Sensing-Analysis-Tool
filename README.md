...
**Note on structure:** the entire application lives in a single `app.py` file (no separate `modules/` package). This is deliberate — it removes the most common Streamlit Cloud deployment failure, where a subfolder gets left out of a GitHub upload/commit and the app fails with `ModuleNotFoundError: No module named 'modules'`. If you still hit an import error, it means a package in `requirements.txt` failed to install — check "Manage app -> Logs" for the pip install step, and confirm `requirements.txt` and `runtime.txt` (pins Python 3.11) are present at the repo root alongside `app.py`.
