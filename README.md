...
**Troubleshooting `ModuleNotFoundError` on Streamlit Cloud:** confirm the deployed repo contains both `app.py` and the `modules/` folder at the same level, that `requirements.txt` matches the one in this project exactly, and that `runtime.txt` (pins Python 3.11) is present at the repo root. Check "Manage app -> Logs" for the pip install step — any red error there means a package failed to build, which is the most common cause of this error.
